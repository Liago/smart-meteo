 ...
 *
 * Portato da `scripts/verifyAlertGeo.ts` alla suite Jest nella Fase 6B: due sistemi di test
...
describe('alertGeo — filtro geografico allerte', () => {
	...
	function alert(partial: Partial<WeatherAlert>): WeatherAlert {
		...
		it('allerte di Emilia e Romagna, Molise e Umbria scartate a Bormio', () => {
		...
		it('allerte della Lombardia mantenute a Bormio', () => {
		const a = alert({
			areaName: 'Lombardia',
		...
		it('EMMA_ID ha la precedenza sul testo libero', () => {
		...
		it('il nome regione va riconosciuto anche con trattino o in inglese', () => {
		...
		it('allerta senza informazioni di area (OpenWeatherMap) mantenuta', () => {
		const a = alert({
		...
		it('la deduplicazione multi-source resta attiva e tiene la severity più alta', () => {
