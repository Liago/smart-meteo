...
- **Parità iOS raggiunta** (14/09, dopo il primo giro su Xcode): `SolarPanelView`, `SkyPanelView`, `SeaPanelView` e `ActivitiesPanelView` portano su iOS le quattro feature che erano solo web. I modelli Swift non decodificavano affatto `solar`, `sky`, `sea` e `activities`: i dati arrivavano e venivano scartati. La potenza dell'impianto fotovoltaico sta in `@AppStorage`, come sul web sta in `localStorage`, e per la stessa ragione: se entrasse nella richiesta, la cache della previsione si frammenterebbe per utente. **Correzione a un conto sbagliato di questo documento:** le feature indietro erano quattro, non cinque — l'orto era già su iOS
- **Ridisegno iOS** (14/09, dal bundle `ios Redisign/`): da tredici pannelli impilati tutti uguali a una gerarchia sola — hero a tinta della condizione più quattro sezioni richiudibili. I sette pannelli contestuali diventano schede in «Per te» che l'utente accende, con una schermata per toglierle e riordinarle. **Scostamento dichiarato dal handoff:** la sua variante «Foglio» perde del tutto il nowcast al minuto, che è l'informazione più urgente dell'app; reintegrato come riga singola nell'hero. Il ridisegno ha anche unificato la mappa codice WMO → icona, che viveva copiata in quattro viste
- **Prima suite di test iOS** (14/09): sei file XCTest. Il più importante è quello di decodifica, perché è lì che il progetto si è già fatto male: una chiave snake_case sbagliata **compila benissimo**, lascia il campo a nil e fa sparire un riquadro senza alcun errore — è esattamente così che `solar`, `sky`, `sea` e `activities` arrivavano e venivano buttati via. Il `project.pbxproj` non è stato toccato a mano di proposito: aggiungere un target alla cieca significa inventare UUID su otto sezioni di un plist, e un errore rende il progetto non apribile. In Xcode è mezzo minuto. **I test non sono mai stati eseguiti**: niente toolchain Swift qui
...
| iOS unit test | ◑ 6 file XCTest scritti, mai eseguiti | Manca il target, da creare una volta in Xcode: `smart-meteoTests/README.md` |
...
> **Prossimo blocco: Fase 6E punto 27** della `GAP_ANALYSIS_2026-09.md` — test iOS e audit Lighthouse, nessuno dei due eseguibile in questo ambiente (niente toolchain Swift, `next build` bloccato dai font). Il punto 17, il radar, resta bloccato. **Il debito iOS è chiuso il 14/09:** i tre client mostrano ora gli stessi blocchi. Restano due asimmetrie *volute*: gli avvisi su soglie personali sono solo iOS (il web non ha push) e il pannello qualità dell'aria in dettaglio è solo web.
