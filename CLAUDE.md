...
│   │   ├── dashboard/InsightsGrid.tsx # "Per te" cards, ordered by relevance
│   │   ├── ui/Modal.tsx, ui/MetricSelect.tsx, ui/SegmentedTabs.tsx # Shared primitives
...
│   │   ├── dashboard.ts        # Dashboard IA: sections + "Per te" relevance ranking
│   │   ├── useDashboardTab.ts  # Open section, synced with the URL hash
...
│   ├── __tests__/              # 17 Jest suites (api, components, weather-utils,
│   │                           #  air-quality, narrative, hourly-detail,
│   │                           #  next-hour, pollen, snow, storm, garden,
│   │                           #  solar, sky, sea, activities, dashboard)
...
│               │   ├── MoonPhase.swift          # Phase from backend name or synodic age, days to full
...
│               │   │   ├── ForYouCard.swift          # Card model + builder from the response (8 cards, incl. Luna)
...
- E2E: `cd frontend-web && npm run test:e2e` - 56 scenarios × 2 viewports (Playwright).
  Since the dashboard was reorganised into sections, a scenario that checks content inside
  one of them opens it first with the `openSection` helper in `e2e/fixtures/api.ts`; the
  ones that only need "the page has loaded" use `dashboardReady`, which does not depend on
  any one section (it used to be the sources card, which now lives behind a tab).
...
- iOS tests live in `frontend-ios/smart-meteo/smart-meteoTests/` (XCTest, 8 files: forecast
  decoding, hourly window, sky, sea, solar, activities + garden, units, moon phase + Luna card). They need a **Unit Testing
...
- **The iOS dashboard is themed by the weather, and everything optional is opt-in** (`ios Redisign/README.md`): page, hero, ink and accent come from `WeatherTheme.of(condition)`, so a rainy day does not wear the same cream as a sunny one — colour is the first information to arrive, before the number. Below one hero sit four collapsible sections (Ora per ora, Prossimi giorni, Per te, Fonti dati) in a single white sheet. The seven contextual panels became **cards inside "Per te" that the user switches on**, persisted in `UserDefaults` like the PV plant size and for the same reason. The rule is still double: a card shows only if the user wants it **and** the backend sent the block, so the sea still disappears inland and the snow in summer. The eighth card, **Luna** (2026-09-17), is the exception on the second half of the rule: phase, illumination, moonrise/moonset and days to the full moon were in the old `CurrentWeatherView` and the redesign had simply lost them; the phase is computed locally (`MoonPhase.swift`, same algorithm as `backend/utils/moon.ts`) when the backend's `moon_phase` is missing or unrecognised — it accepts both the Italian names and the English ones the moonrise-bearing provider sends — so the card exists every night. `daysToFull` returns 0 for the whole «full» phase (±1.85 days), or the night after the full moon would read «Luna piena · piena fra 28 giorni». A card added after a user has already saved their choices would stay off forever (the saved list does not name it), so `ForYouPrefs.enabled(_:known:)` turns on the on-by-default cards the settings screen has never shown; the settings screen records `knownKey` on every toggle or move, after which "off" means "left off on purpose". **The hero is SF Pro only** (same date): temperature in `.light` and the narrative line, both previously New York serif, read as two fonts picked at random above the SF Pro labels and were reported as a defect; the serif stays on the section titles and the secondary sheets, where it stands alone. The compass label in the narrative («brezza da SSW») is uppercased: it is a cardinal point, not a word. Card text reuses the original panels' static helpers rather than restating it — two screens describing the same datum in two wordings are worse than one screen. Two hero variants ship (`HeroVariant`, switchable in settings) because the handoff offered both and they differ only in the hero: "Foglio" gives the temperature the whole stage, "Tessere" trades some of it for wind, UV, nowcast and sunset at a glance
