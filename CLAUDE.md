...
- **A bar is drawn from the axis floor, not from zero** (`HourlyDetailView` + `MetricSection.barBounds`): Swift Charts' `BarMark(x:y:)` anchors the column at **zero** and does not clip marks to the plot area, so the one bar metric whose domain does not start at zero — «Percepita», because a day between 18 and 24 °C on an axis starting at 0 would be a row of near-identical bars — had its columns starting ~200 points below the chart and painting **over** the summary tiles and the closing note. The domain was never the defect: assuming the axis floor was zero was. The web never had it because its hand-written SVG draws from an explicit `baselineY`, which *is* the domain floor. Values are clamped into the domain as the web clamps them (a source reporting 105% humidity shortens the bar to the edge instead of leaving the box) while the **colour still comes from the true value**, since classification is not geometry; and the plot area is clipped as a safety net, so a future mark out of domain stays inside the chart where it is visible rather than on top of the text. Related, same family: the zero-based domains' guaranteed floor is now evaluated **at draw time** — the sections are `static let`, initialised once per process, so a floor computed inside them froze in whatever unit was in effect at first access while the band labels converted on every call, and switching to knots gave an axis 50 **knots** tall with the bars squashed at the bottom
...
- **Barre del dettaglio orario dentro il grafico** (2026-09-17, iOS): segnalato da uno screenshot della metrica «Percepita», con le colonne che proseguivano sotto il grafico fin sopra le tessere di riepilogo. Una riga di causa — `BarMark(x:y:)` ancora a zero, non al fondo dell'asse, e Swift Charts non ritaglia — su un dominio che per una temperatura è giusto **non** parta da zero. Corretto ancorando la barra al fondo del dominio come fa l'SVG del web da sempre, limitando i valori fuori scala e ritagliando l'area del grafico. Sulla stessa strada è venuto fuori che il minimo garantito degli assi che partono da zero era congelato nell'unità del primo disegno. Nuova suite `MetricScaleTests`.
